...
function BodyReader(options) {
    ...
    var xmlElementReaders = {
        ...
        "m:oMathPara": readOMML // 遇到m:oMathPara标签后调用readOMML处理
    };
    ...
    function readOMML(element) {
        // 将含有omml的element重新还原成xml字符串
        var docEl = new xml.Element('w:document', {}, [element])
        var xmlStr = xml.writeString(docEl, {
            'xml': 'http://www.w3.org/XML/1998/namespace',
            'w': 'http://schemas.openxmlformats.org/wordprocessingml/2006/main',
            'm': 'http://schemas.openxmlformats.org/officeDocument/2006/math'
        });

        // 再将xml字符串解析成xmldom，因为omml2mathml支持xmldom
        const doc = new DOMParser().parseFromString(xmlStr, 'text/xml');
        var mathmlEl = omml2mathml(doc)

        // 再将mathml转成latex格式
        console.log('++++++++++++++++++++34523452')
        var latex = MathMl2LaTeX.convert(mathmlEl.outerHTML)

        return elementResult(new documents.MathML({mathml: mathmlEl.outerHTML}));
