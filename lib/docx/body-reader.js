...
function BodyReader(options) {
    ...
    var xmlElementReaders = {
        ...
        "m:oMathPara": readOMML, // OMML 数学公式段落
        "m:oMath": readOMML // OMML行内数学公式
    };
    ...
    function readOMML(element) {
        ...
        // 再将xml字符串解析成xmldom，因为omml2mathml支持xmldom做为参数
        const doc = new DOMParser().parseFromString(xmlStr, 'text/xml');
        var mathmlEl = omml2mathml(doc)

        // 向在文档中生成新的mathml元素
        return elementResult(new documents.MathML({
            mathml: mathmlEl.outerHTML
        }));
